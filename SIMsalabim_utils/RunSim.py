...
def PrepareSimuInputs(Path2Simu,parameters=None,fixed_str=None,CodeName = 'SimSS',SaveVarFile = True,verbose=False):
    ...
    if len(parameters) > 1:
        ...
        if SaveVarFile:
            str_lst.append(fixed_str+ ' -JV_file JV.dat -Var_file Var.dat -scPars_file scPars.dat')
        else:
            str_lst.append(fixed_str+ ' -JV_file JV.dat -Var_File none -scPars_file scPars.dat')
...
def  DegradationPrepareSimuInputs(Path2Simu,parameters=None,fixed_str=None,CodeName = 'SimSS',SaveVarFile=True,verbose=False):
    """Prepare the command strings to run and the fixed parameters and output files
    This procedure will make command with fixed_str and all the possible combinations of the input parameters in parameters. (see itertools.product)
...
def  DegradationPrepareSimuInputs_light_dep(Path2Simu,parameters=None,fixed_str=None,CodeName = 'SimSS',SaveVarFile = True ,verbose=False):
    """Prepare the command strings to run and the fixed parameters and output files
    This procedure will make command with fixed_str and all the possible combinations of the input parameters in parameters. (see itertools.product)
    ...
    # Check if 'Gfrac' is in the parameters list
    if 'Gfrac' in [i['name'] for i in parameters]:
        # store the values of Gfrac
        Gfrac = [i['values'] for i in parameters if i['name'] == 'Gfrac'][0]
        # remove Gfrac from the parameters list
        parameters = [i for i in parameters if i['name'] != 'Gfrac']
    else:
        # stop the simulation if Gfrac is not in the parameters list
        raise ValueError('Gfrac must be in the parameters list')
    ...
    if len(parameters) >= 1:
        ...
        for index, row in param2run.iterrows():
            ...
            for G in Gfrac:
                ...
                Var_name = 'Var'
                scPars_name = 'scPars'
                str_line = str_line +'-Gfrac {:.2e} '.format(G)
                out_name = out_name +'_Gfrac_{:.2e}'.format(G)
                Var_name = Var_name +'_Gfrac_{:.2e}'.format(G)
                scPars_name = scPars_name +'_Gfrac_{:.2e}'.format(G)
                lab = lab+'Gfrac {:.2e} '.format(G)
                for name in param2run.columns:
                    str_line = str_line +'-'+name+' {:.2e} '.format(row[name])
                    lab = lab+name+' {:.2e} '.format(row[name])
